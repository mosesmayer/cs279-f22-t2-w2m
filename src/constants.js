...
// for fmting purposes, ensure minute is 0
const START_TIME = [9, 0]; // starting [hour, minute] -> ensure minute is 0
const END_TIME = [17, 0];  // ending [hour, minute]   -> ensure minute is 0
const START_DATE = [2022, 11, 7] // starting [y, m, d]
const END_DATE = [2022, 11, 11] // ending [y, m, d]
// const maxRows = 3;
const maxRows = (END_TIME[0] - START_TIME[0]) * 4;
// const maxCols = 4;
const maxCols = END_DATE[2] - START_DATE[2]; // fix this calculation later

const starting_num_attendees = 5;
const starting_grid = [];
    ...
    for (let j = 0; j < maxCols; j++) {
        for (let i = 0, ii = 0; ii < maxRows; i = ii) {
            if (Math.random() < 0.4) {
                ...
                // console.log("Add from ", i, j, " to ", ii, j);
...
export { START_TIME, END_TIME, START_DATE, END_DATE }
